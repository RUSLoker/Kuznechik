...
block_t MSB(uint64_t n, block_t block);
...
void bit_vec_append(bit_vec_t *bit_vec, const block_t data, const uint64_t size) {
    ...
    const uint64_t last_incomplete_size = bit_vec->bit_size % BLOCK_BIT_LEN;

    const uint64_t last_incomplete_idx = bit_vec->bit_size / BLOCK_BIT_LEN;

    block_t shifted_data = data;
    uint64_t data_remained_size = size;

    if (last_incomplete_size != 0) {
        const uint64_t last_block_free_size = BLOCK_BIT_LEN - last_incomplete_size;

        // size of the part less than the size of a block need to be taken from data
        const uint64_t first_incomplete_size = MIN(last_block_free_size, size);
    ...
    if (data_remained_size > 0) {
        const uint64_t next_block_idx = bit_vec->bit_size / BLOCK_BIT_LEN;
...
void MGM_apply_cypher(key256_t key, block_t nonce, bit_vec_t plain_text, bit_vec_t *cyphered_text) {
    ...
    const uint64_t incomplete_size = plain_text.bit_size % BLOCK_BIT_LEN;
...
block_t MSB(uint64_t n, block_t block) {
    return block >> (BLOCK_BIT_LEN - n) << (BLOCK_BIT_LEN - n);
}
